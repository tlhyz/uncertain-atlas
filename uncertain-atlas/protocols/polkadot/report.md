...
3. 一组验证者 backing：声称「我们检查过」。**Backed 仍可能进不了平行链。**  
4. 候选数据必须可用，否则无法独立复验。中继头上的是回执，PoV / 纠删片不在中继状态。可用不保证有效。  
5. 其他验证者可能被抽中重新执行（审批）。失败则争议；included 仍 pending approval。  
6. 中继链 GRANDPA 最终后，该平行块才算在共享安全下敲定。批准不是已经最终。  
7. 若只听 collator 的 RPC、不等中继最终：用户层假确认。精读：[`../../tracks/finality/worked-example-backed-vs-available.md`](../../tracks/finality/worked-example-backed-vs-available.md)（不变量 125）。
...
PoV 可能很大：可用性协议把数据分散。纠删片在验证者磁盘，不在中继块。  
攻击：只让 backing 小组看见数据，其他人抽查失败或无法重建——这是 DA 问题在共享安全里的翻版。这和 Celestia 轻节点 DAS 不是同一对象。

---

## 9. 存储

中继存元数据与最终头。官方：回执进中继；纠删片在验证者磁盘，有时限。平行状态主要由该链节点存。  
归档谁负责「能复验旧平行块」，要单独设计。不要把「曾经可用」写成历史已经永存。
...
1. 候选 backing（回执，不是平行块字节）。  
2. 可用性比特与重建（磁盘上的片；与 Backed 不是同一谓词）。  
3. approval 抽查（included 仍 pending approval）。
