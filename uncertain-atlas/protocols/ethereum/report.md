...
devp2p / discv5 等。块与 blob（EIP-4844 后）传播是新带宽账。看见忽略 hello / ping 版本或多余字段不是已经谈成新线协议；看见跟 Homestead 一起上不是已经改了共识；看见仍收旧握手不是已经退役旧格式：[`../../tracks/network/worked-example-eip8-vs-already-new.md`](../../tracks/network/worked-example-eip8-vs-already-new.md)（不变量 235）。看见分叉标识对上不是已经同一条链；看见通告了下一分叉不是已经兼容：[`../../tracks/network/worked-example-forkid-vs-same-chain.md`](../../tracks/network/worked-example-forkid-vs-same-chain.md)（不变量 239）。  
