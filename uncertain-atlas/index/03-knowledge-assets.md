...
| 03 | 《共识协议图谱》 | `tracks/consensus/` + `tracks/finality/` | 对照表 + ABCI 四门 + 扩展 + 集合延迟 + PBTS + Bitcoin MTP 三把尺 + 本地超时 ≠ 最终性 + 应用 delay ≠ 槽位 + 默认 MaxBytes ≠ 第一轮 SLA + backed ≠ 可用 + BABE ≠ GRANDPA + head ≠ justified ≠ finalized + NPoS 等权 ≠ 按质押凑 ⅔ + leak ≠ slash + 抽样 α ≠ QC + 已认证批次 ≠ L + PoH ≠ Tower + VRF 抽中 ≠ 已认证 + Doomslug ≠ BFT 谓词 + 顺序 ≠ 状态根 + DAG ≠ selected chain + L2 档 ≠ L1 + unsafe ≠ 已推导 + 本头 AppHash ≠ 本块已交差 | 必学 |
| 04 | 《状态模型图谱》 | `tracks/state-models/` + `tracks/parallelism/` | 进行中；Block-STM 工作实例（不变量 122）；owned ≠ 快路径（不变量 128）；Quorum Store ≠ 排序（不变量 132）；顺序 ≠ 状态根（不变量 136）；谓词 ≠ 脚本 / 只读重叠 ≠ 写冲突（不变量 143） | 必学 |
...
| 07 | 《Design Pattern Library》 | `libraries/design-patterns/` | 41 条 | 重要 |
| 08 | 《Anti-Pattern Library》 | `libraries/anti-patterns/` | 148 条 | 重要 |
...
| 12 | 《不确定 Invariant Library》 | `libraries/invariants/` | 147 条 | 必学 |
| 13 | 《不确定 Adversarial Test Corpus》 | `libraries/adversarial-corpus/` | 目录 C01–C151；runner 未建 | 重要 |
...
| 链 / 品类 | 波次 | 优先级 | 档案状态 |
|---|---|---|---|
| Bitcoin | 1 | 必学 | 第一版；策略 ≠ 共识（不变量 144） |
| Cosmos / CometBFT | 2 | 必学 | 第一版；IBC 客户端 ≠ 数据包（不变量 146）；本头 AppHash ≠ 本块已交差（不变量 147） |
| Ethereum | 3 | 重要 | 第一版；blob gas ≠ 执行气（不变量 145） |
