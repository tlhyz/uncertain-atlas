...
| 03 | 《共识协议图谱》 | `tracks/consensus/` + `tracks/finality/` | 对照表 + ABCI 四门 + 扩展 + 集合延迟 + PBTS + Bitcoin MTP 三把尺 + 本地超时 ≠ 最终性 + 应用 delay ≠ 槽位 + 默认 MaxBytes ≠ 第一轮 SLA + backed ≠ 可用 + BABE ≠ GRANDPA + head ≠ justified ≠ finalized + NPoS 等权 ≠ 按质押凑 ⅔ + leak ≠ slash + 抽样 α ≠ QC + 已认证批次 ≠ L + PoH ≠ Tower + VRF 抽中 ≠ 已认证 + Doomslug ≠ BFT 谓词 + 顺序 ≠ 状态根 + DAG ≠ selected chain + L2 档 ≠ L1 + unsafe ≠ 已推导 + 本头 AppHash ≠ 本块已交差 + 本头 LastCommit ≠ 本高已 +2/3 | 必学 |
...
| 07 | 《Design Pattern Library》 | `libraries/design-patterns/` | 42 条 | 重要 |
| 08 | 《Anti-Pattern Library》 | `libraries/anti-patterns/` | 149 条 | 重要 |
...
| 12 | 《不确定 Invariant Library》 | `libraries/invariants/` | 148 条 | 必学 |
| 13 | 《不确定 Adversarial Test Corpus》 | `libraries/adversarial-corpus/` | 目录 C01–C152；runner 未建 | 重要 |
