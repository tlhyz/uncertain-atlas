...
| 07 | 《Design Pattern Library》 | `libraries/design-patterns/` | 52 条 | 重要 |
| 08 | 《Anti-Pattern Library》 | `libraries/anti-patterns/` | 159 条 | 重要 |
...
| 12 | 《不确定 Invariant Library》 | `libraries/invariants/` | 158 条 | 必学 |
| 13 | 《不确定 Adversarial Test Corpus》 | `libraries/adversarial-corpus/` | 目录 C01–C162；runner 未建 | 重要 |
...
| Ethereum | 3 | 重要 | 第一版；blob gas ≠ 执行气（不变量 145）；处理完一块 ≠ 已经改规范头（不变量 149）；提款操作 ≠ 用户交易（不变量 154）；父信标根 ≠ 当前头（不变量 156）；PREVRANDAO ≠ 工作量 / 无偏骰子（不变量 157）；基础费 ≠ 小费 / 烧掉 ≠ MEV 已解决（不变量 158） |
