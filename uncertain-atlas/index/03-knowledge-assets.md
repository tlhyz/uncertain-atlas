...
| 04 | 《状态模型图谱》 | `tracks/state-models/` + `tracks/parallelism/` | 进行中；Block-STM 工作实例（不变量 122）；owned ≠ 快路径（不变量 128）；Quorum Store ≠ 排序（不变量 132）；顺序 ≠ 状态根（不变量 136）；谓词 ≠ 脚本 / 只读重叠 ≠ 写冲突（不变量 143）；引用输入 ≠ 已经花费（不变量 150）；`store` ≠ 顶层资源（不变量 151）；瞬时存储 ≠ 账户持久存储（不变量 159）；后来的 SELFDESTRUCT ≠ 账户已经删掉（不变量 160）；发送者已有代码 ≠ 已经能当 EOA 发交易（不变量 162） | 必学 |
...
| 07 | 《Design Pattern Library》 | `libraries/design-patterns/` | 57 条 | 重要 |
| 08 | 《Anti-Pattern Library》 | `libraries/anti-patterns/` | 164 条 | 重要 |
...
| 12 | 《不确定 Invariant Library》 | `libraries/invariants/` | 163 条 | 必学 |
| 13 | 《不确定 Adversarial Test Corpus》 | `libraries/adversarial-corpus/` | 目录 C01–C167；runner 未建 | 重要 |
...
| 链 / 品类 | 波次 | 优先级 | 档案状态 |
|---|---|---|---|
| Bitcoin | 1 | 必学 | 第一版；策略 ≠ 共识（不变量 144）；txid ≠ wtxid（不变量 152）；钥匙路径 ≠ 揭树（不变量 153）；进了块的 coinbase ≠ 已经能花（不变量 163） |
