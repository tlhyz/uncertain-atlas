...
| 04 | 《状态模型图谱》 | `tracks/state-models/` + `tracks/parallelism/` | 进行中；Block-STM 工作实例（不变量 122）；owned ≠ 快路径（不变量 128）；Quorum Store ≠ 排序（不变量 132）；顺序 ≠ 状态根（不变量 136）；谓词 ≠ 脚本 / 只读重叠 ≠ 写冲突（不变量 143）；引用输入 ≠ 已经花费（不变量 150） | 必学 |
...
| 07 | 《Design Pattern Library》 | `libraries/design-patterns/` | 44 条 | 重要 |
| 08 | 《Anti-Pattern Library》 | `libraries/anti-patterns/` | 151 条 | 重要 |
...
| 12 | 《不确定 Invariant Library》 | `libraries/invariants/` | 150 条 | 必学 |
| 13 | 《不确定 Adversarial Test Corpus》 | `libraries/adversarial-corpus/` | 目录 C01–C154；runner 未建 | 重要 |
