...
| 03 | 《共识协议图谱》 | `tracks/consensus/` + `tracks/finality/` | 对照表 + ABCI 四门 + 扩展 + 集合延迟 + PBTS + Bitcoin MTP 三把尺 + 本地超时 ≠ 最终性 + 应用 delay ≠ 槽位 + 默认 MaxBytes ≠ 第一轮 SLA + backed ≠ 可用 + BABE ≠ GRANDPA + head ≠ justified ≠ finalized + NPoS 等权 ≠ 按质押凑 ⅔ | 必学 |
...
| 07 | 《Design Pattern Library》 | `libraries/design-patterns/` | 23 条 | 重要 |
| 08 | 《Anti-Pattern Library》 | `libraries/anti-patterns/` | 132 条 | 重要 |
...
| 12 | 《不确定 Invariant Library》 | `libraries/invariants/` | 129 条 | 必学 |
| 13 | 《不确定 Adversarial Test Corpus》 | `libraries/adversarial-corpus/` | 目录 C01–C133；runner 未建 | 重要 |
