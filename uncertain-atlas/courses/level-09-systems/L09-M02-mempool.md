...
**禁止假学习：** 「mempool 是全球公平队列。」「进池 = 结算。」「策略拒绝 = 共识非法。」「跳过库存通告 = 已经拒进池。」「发了费率过滤器 = 对等节点已经照做。」「对账素描 = 已经有交易。」「对账失败退回洪水 = 库存通告已经退役。」「内存池查询回了一串库存 = 已经有那些交易。」「只肯给最近转发过的 = 已经支持整池查询。」「协议版本够了 = 已经在答内存池查询。」「拒收消息 = 已经共识非法。」「调试理由 = 已经该给用户看。」「没拒收 = 已经是当前最好链。」
