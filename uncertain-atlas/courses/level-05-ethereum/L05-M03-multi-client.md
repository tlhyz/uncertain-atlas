...
看见协议自己写死的 RLP 编码硬帽，不是已经改了气限，也不是已经修了各家出块通道尺寸。Sepolia 那次是 Engine API 沿用 HTTP 上限（不变量 96）。EIP-7934 是另一道、独立于气的编码谓词。精读：[`../../tracks/implementation/worked-example-rlp-cap-vs-gas.md`](../../tracks/implementation/worked-example-rlp-cap-vs-gas.md)（不变量 202）。
