...
| 2.2 | [L02-M02-account.md](L02-M02-account.md) | 账户 + nonce 换来了什么；可花 ≠ 锁定已从写回排除；瞬时存储 ≠ 账户持久存储（不变量 159）；后来的 SELFDESTRUCT ≠ 账户已经删掉（不变量 160）；发送者已有代码 ≠ 已经能当 EOA 发交易（不变量 162）；nonce 顶到规范上限 ≠ 已经还能加一（不变量 175）；空 ≠ 已经不存在（不变量 180） |
