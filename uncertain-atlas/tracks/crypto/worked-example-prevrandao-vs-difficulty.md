...
6. **历史执行哈希 ≠ BLOCKHASH**（不变量 195）：2935 是执行块哈希进状态。本页是 RANDAO mix。看见状态槽不是已经是本页。

不要抄过渡阈值 / 字段下标 / 前瞻 epoch / 漏块率。不要写怎样扣块、怎样审查掷骰交易、怎样叠 1 bit。不编博物馆页。不另写 19 节。RANDAO 揭示公式、应用 commit-reveal、`BLOCKHASH` 当随机源标成另一对象。EIP-2935 历史执行哈希见不变量 195。出块人前瞻不是已经是本页（205）。
