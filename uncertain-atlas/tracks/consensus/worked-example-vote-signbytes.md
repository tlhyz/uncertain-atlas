...
Builder API 的登记/出价走 `DOMAIN_APPLICATION_BUILDER`（`compute_domain` 默认参数）；盲块本身仍走 `DOMAIN_BEACON_PROPOSER`。域分不出「谁写的交易列表」，见 [谁排序](../mempool/worked-example-who-orders.md)。
