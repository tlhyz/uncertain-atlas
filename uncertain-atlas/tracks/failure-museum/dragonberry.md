...
亲戚 **Elderflower**（同一复盘、同一天公开发布，**不另立不变量**）：Authz 消息管道在 SDK v0.44 / 0.45 / 0.46 **漏掉**一次 `ValidateBasic()`，可能造出无效状态转移，在「恰好的交易参数」下或可通胀或盗窃。已在未发布的 `main` 修过。对象是模块认证，不是 ICS-23 soundness。
...
7. Elderflower 是另一对象（Authz 漏 `ValidateBasic`）。不要把「同一天发布」写成同一谓词。不要抄 authz。
