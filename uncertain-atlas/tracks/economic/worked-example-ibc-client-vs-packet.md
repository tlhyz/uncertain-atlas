...
不要把握手步数、超时高度、ibc-go 版本抄进不确定常量。不要写怎样构造包、怎样扣包、怎样嵌套 ICS-20。不编博物馆页。不另写 19 节。ICS-20 应用、通道升级、WASM 客户端标成另一对象。
...
**边界：** 不讲某一版 ibc-go 的 keeper 函数表。不抄握手超时。不另写 19 节。不写怎样构造包。ICS-20 / hooks / ICS-23 / 通道升级另标。
