...
对等历史窗 ≠ 已改共识：[`worked-example-history-window-vs-consensus.md`](worked-example-history-window-vs-consensus.md)（看见对等节点宣布历史窗不是已经删历史；线上无布隆不是已经改共识收据；7642 ≠ 23 ≠ 25 ≠ 195）。EIP-7642 history-window not already consensus-pruned / not already 23 / not already 207-bundled 正式三事（207 余量）：[`worked-example-hwin-notcons-vs-bundled.md`](worked-example-hwin-notcons-vs-bundled.md)（不变量 1488）。EIP-7642 wire-no-bloom not already consensus-receipt / not already 25 / not already 195 正式三事（207 余量）：[`worked-example-hwin-notenc-vs-bundled.md`](worked-example-hwin-notenc-vs-bundled.md)（不变量 1489）。EIP-7642 handshake-no-td not already sync-done / not already safer / not already 167 正式三事（207 余量）：[`worked-example-hwin-notsync-vs-bundled.md`](worked-example-hwin-notsync-vs-bundled.md)（不变量 1490）。  
忽略 hello 版本 ≠ 已经在说新协议：[`worked-example-eip8-nothello-vs-bundled.md`](worked-example-eip8-nothello-vs-bundled.md)（忽略 hello 版本≠already 在说新协议/谈成新线协议/新功能启用；≠235 bundled；不变量 1283）。
...
协议版本 ≠ 已经是客户端版本：[`worked-example-ua14-notcli-vs-bundled.md`](worked-example-ua14-notcli-vs-bundled.md)（协议版本≠already 是客户端版本/是实现版本/抬了协议；≠263 bundled；不变量 1271）。
user agent ≠ 已经可以按实现改行为：[`worked-example-ua14-notua-vs-bundled.md`](worked-example-ua14-notua-vs-bundled.md)（user agent≠already 可以按实现改行为/可以按这家实现换规则/是本页允许的行为；≠263 bundled；不变量 1272）。
同一协议版本 ≠ 已经是同一套实现：[`worked-example-ua14-notsame-vs-bundled.md`](worked-example-ua14-notsame-vs-bundled.md)（同一协议版本≠already 是同一套实现/是协议能力/谈妥了某项功能；≠263 bundled；不变量 1273）。
