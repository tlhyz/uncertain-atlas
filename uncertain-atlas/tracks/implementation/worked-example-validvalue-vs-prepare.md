...
**不要写进**：Ethereum 行、L5.1、M5.4、L5.4、03 共识图谱、Bitcoin 行、L4.5。本页是「validValue 非 nil 不是已经还会调 Prepare / 自己是提议者不是已经每轮都会调 Prepare / 没调 Prepare 不是已经又从池子装了一份 raw 提案」，不是候选已经是 ExecuteTxState，也不是 Prepare 没有确定性要求。不要另写怎样设 validValue。356 validvalue vs prepare bundled unbundling 完成（821+822+823）；精读 [`worked-example-vv-notstillprepare-vs-bundled.md`](worked-example-vv-notstillprepare-vs-bundled.md)（不变量 821 item 1）；精读 [`worked-example-vv-noteveryround-vs-bundled.md`](worked-example-vv-noteveryround-vs-bundled.md)（不变量 822 item 2）；精读 [`worked-example-vv-notraw-vs-bundled.md`](worked-example-vv-notraw-vs-bundled.md)（不变量 823 item 3）。
...
**建议（产品，不是事实）**：不确定第一条结算机如果给人看「看见本轮直接用它就已经还会调 Prepare、已经每轮都会调、已经又装了一份 raw 提案」，必须分开 validValue 非 nil 是不是已经还会调 Prepare、自己是提议者是不是已经每轮都会调 Prepare、没调 Prepare 是不是已经又装了一份 raw 提案。可以跳过「看见本轮直接用它就已经还会调 Prepare」。不要另写怎样设 validValue。356 validvalue vs prepare bundled unbundling 完成（821+822+823）。
