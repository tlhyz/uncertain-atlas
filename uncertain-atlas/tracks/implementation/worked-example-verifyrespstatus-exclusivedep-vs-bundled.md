...
怎样做写 Verify 回包栏、怎样测确定性 是规范里的做法，本页不抄。Verify 回包栏 bundled（433）、Verify 341 determinism（341）、status valid/invalid（539 余量）是另外那套，本页不抄。
...
- VerifyVoteExtensionResponse.status valid/invalid 语义。那是不变量 539（433 item 1 余量）。
