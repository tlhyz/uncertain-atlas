...
怎样写 FinalizeBlock、怎样测确定性、怎样写测试向量是规范里的做法，本页不抄。FinalizeBlock 套用候选（460）是 executes txs / apply candidate / 先前 Prepare Process 执行过那套另一切片，Finalize 回包 app_hash 可以空或硬编码 bundled（404）是 app_hash / Query proofs / tx_results Code==0 那套另一切片，FinalizeBlockResponse next_block_delay 非确定（469）是 next_block_delay 非确定 / wallclock / Commit 后再开下一高那套另一切片，FinalizeBlock 确定性 Req 11–12（342）是 s_h / T_h / 状态机复制那套来自 app requirements 的另一切片，Finalize 字段余量 bundled（407）是 finfields / MUST be deterministic / Info 那套另一切片，本页不抄。
...
| app_hash MUST be deterministic / only params + previous state | 不是已经印进本头 | 不是 next_block_delay 非确定（469） |
