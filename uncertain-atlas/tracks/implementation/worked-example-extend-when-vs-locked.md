...
**不要写进**：Ethereum 行、L5.1、M5.4、L5.4、03 共识图谱、Bitcoin 行、L4.5。本页是「收到提案和全部块片并且 +2/3 prevote 同一 id(v) 才锁住再调 ExtendVote 不是已经会调 ExtendVote / ExtendVote 调用是同步的不是已经能在返回之后再改扩展 / 应用回了一串字节共识算法不解释不是已经是同一份扩展」，不是一轮只能交出一份扩展，也不是 Process 调用是同步的。不要另写怎样写 ExtendVote 何时调用。361 extend-when vs locked bundled unbundling 续（833+834）；精读 [`worked-example-extwhen-notwillcall-vs-bundled.md`](worked-example-extwhen-notwillcall-vs-bundled.md)（不变量 833 item 1）；精读 [`worked-example-extwhen-notlaterrevise-vs-bundled.md`](worked-example-extwhen-notlaterrevise-vs-bundled.md)（不变量 834 item 2）。
