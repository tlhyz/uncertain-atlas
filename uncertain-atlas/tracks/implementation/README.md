- [`worked-example-queryprove-notkey-vs-bundled.md`](worked-example-queryprove-notkey-vs-bundled.md) — 回了证明≠已是按键查/有序列化证明≠已对上 AppHash/能回≠已交差；≠383 bundled（不变量 897）
...
- [`worked-example-infover-notappversion-vs-bundled.md`](worked-example-infover-notappversion-vs-bundled.md) — 填了 version≠已是 app_version/有软件版本≠已印进本头 AppHash/能回≠已交差；≠379 bundled（不变量 884）
- [`worked-example-refetch-notproceed-vs-bundled.md`](worked-example-refetch-notproceed-vs-bundled.md) — 拒了人≠已能接着装/丢掉排队≠已停/已装的还在≠已齐；≠378 bundled（不变量 883）
- [`worked-example-refetch-notcomplete-vs-bundled.md`](worked-example-refetch-notcomplete-vs-bundled.md) — 列了块号≠已齐/再装≠已交差/按顺序≠已是同一份；≠378 bundled（不变量 882）
...
- [`worked-example-preparetimeout-notlivenesslost-vs-bundled.md`](worked-example-preparetimeout-notlivenesslost-vs-bundled.md) — 又开一轮≠丢活性/初值≠超时冻住/看见初值≠末档；≠327 bundled（不变量 739）
...
- [`worked-example-paramsdelay-notveheight-vs-bundled.md`](worked-example-paramsdelay-notveheight-vs-bundled.md) — 参数更新写了H+1≠扩展启用高度切换/立刻生效≠只改填的那一项/写了H+1就当成切换≠切到ABCI2.0；≠333 bundled（不变量 757）
...
- [`worked-example-verify-nothonestonly-vs-bundled.md`](worked-example-verify-nothonestonly-vs-bundled.md) — 两边同判≠只对诚实扩展/扩展坏了≠可以各判各的/任意扩展≠已经是Req6；≠341 bundled（不变量 777）
- [`worked-example-verify-notlikeextend-vs-bundled.md`](worked-example-verify-notlikeextend-vs-bundled.md) — 必须确定≠可以像ExtendVote那样/只依赖扩展这块和上一份状态≠和ExtendVote nondet同一句/Verify回了≠已经交差；≠341 bundled（不变量 776）
- [`worked-example-process-notlostsafety-vs-bundled.md`](worked-example-process-notlostsafety-vs-bundled.md) — 活性不能保证≠已经丢了安全性/没有现成解法≠已经有补丁/SHOULD Accept≠已经必须拒坏块；≠340 bundled（不变量 775）
- [`worked-example-process-nothonestonly-vs-bundled.md`](worked-example-process-nothonestonly-vs-bundled.md) — 两边同判≠只对诚实提案/提议者坏了≠可以各判各的/任意块≠已经是Req3；≠340 bundled（不变量 774）
- [`worked-example-process-notlikeprepare-vs-bundled.md`](worked-example-process-notlikeprepare-vs-bundled.md) — 必须确定≠可以像Prepare那样/只依赖请求和上一份状态≠和Prepare nondet同一句/Process回了≠已经交差；≠340 bundled（不变量 773）
- [`worked-example-checktx-notprocess-vs-bundled.md`](worked-example-checktx-notprocess-vs-bundled.md) — 有ProcessProposal≠已经是CheckTx/点名了这道门≠已经是Finalize/会拒提案≠池子里挡完；≠339 bundled（不变量 772）
- [`worked-example-checktx-notblocked-vs-bundled.md`](worked-example-checktx-notblocked-vs-bundled.md) — 拜占庭可不管CheckTx≠已被池子挡住/能提案无效块≠进不了共识/诚实过了≠同一把尺；≠339 bundled（不变量 771）
...
- [`worked-example-snapshotconn-notgone-vs-bundled.md`](worked-example-snapshotconn-notgone-vs-bundled.md) — 选择不实现≠没有state sync对象/可选≠从创世唯一/可选≠清单已齐；≠334 bundled（不变量 760）
- [`worked-example-snapshotconn-notbothends-vs-bundled.md`](worked-example-snapshotconn-notbothends-vs-bundled.md) — 给人快照≠必须两头都做/给自己装回≠已经装完/写了和或≠已经两头都做了；≠334 bundled（不变量 759）
...
- [`worked-example-queryproof-nottxmerkle-vs-bundled.md`](worked-example-queryproof-nottxmerkle-vs-bundled.md) — 头上AppHash≠交易默克尔/并列≠验证者集合锚/交易在≠分开应用状态；≠325 bundled（不变量 731）
- [`worked-example-snapshottake-notretained-vs-bundled.md`](worked-example-snapshottake-notretained-vs-bundled.md) — 只留两份≠全部历史/Hash对上≠五字段同一份/有Hash≠轻验AppHash；≠324 bundled（不变量 730）
- [`worked-example-snapshottake-notconsistent-vs-bundled.md`](worked-example-snapshottake-notconsistent-vs-bundled.md) — 没停链≠Consistent/后台拍≠Asynchronous交差/同一高度≠各节点字节相同；≠324 bundled（不变量 729）
- [`worked-example-snapshottake-notcommitted-vs-bundled.md`](worked-example-snapshottake-notcommitted-vs-bundled.md) — 标了高度≠交差之后拍/拍了≠没有更高高度/字段在≠隔离在这一高度；≠324 bundled（不变量 728）
- [`worked-example-snapshotswitch-nothistory-vs-bundled.md`](worked-example-snapshotswitch-nothistory-vs-bundled.md) — 能出块≠完整历史/一样跑≠从创世重放/透明≠不用管扩展高度；≠323 bundled（不变量 727）
- [`worked-example-snapshotswitch-notversion-vs-bundled.md`](worked-example-snapshotswitch-notversion-vs-bundled.md) — AppHash对上≠版本也对上/对了下一高度≠对了当前头/Info绿了≠本头交差；≠323 bundled（不变量 726）
- [`worked-example-snapshotswitch-notchainid-vs-bundled.md`](worked-example-snapshotswitch-notchainid-vs-bundled.md) — 装完≠有ChainID/状态机在≠能出块/有创世文件≠和轻客户端对过；≠323 bundled（不变量 725）
- [`worked-example-snapshotdiscover-notstop-vs-bundled.md`](worked-example-snapshotdiscover-notstop-vs-bundled.md) — 被拒≠没有快照/拒了邻居≠停/能中止≠发现完；≠322 bundled（不变量 724）
...
- [`worked-example-snapshotrestore-notrestored-vs-bundled.md`](worked-example-snapshotrestore-notrestored-vs-bundled.md) — Offer收下≠装完/选了这份≠全部块/元数据对上≠验过AppHash；≠321 bundled（不变量 719）
...
- [`worked-example-validatorupdate-notdup-vs-bundled.md`](worked-example-validatorupdate-notdup-vs-bundled.md) — 看见重复≠按后一条改权/看见失败≠能重放修好/看见同一把钥≠能写两行；≠318 bundled（不变量 714）
- [`worked-example-validatorupdate-notempty-vs-bundled.md`](worked-example-validatorupdate-notempty-vs-bundled.md) — 回了空≠没有集合/没回人≠删掉创世名单/能设初始集合≠应用空集；≠318 bundled（不变量 713）
