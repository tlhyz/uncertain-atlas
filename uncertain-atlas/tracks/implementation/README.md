...
- [`worked-example-offersnaptrust-vs-metadata.md`](worked-example-offersnaptrust-vs-metadata.md) — Only AppHash can be trusted ≠ Snapshot 字段都可信；Any other data can be spoofed / employ additional verification ≠ hash 比对就够 / 已经防 DoS 交差；verified AppHash automatically checked at end ≠ 装块时就 Info 对了 / 已经切进共识（不变量 483）
- [`worked-example-chktxtype-vs-recheck.md`](worked-example-chktxtype-vs-recheck.md) — CheckTx_New default full check ≠ CheckTx_Recheck / tx 栏就等于 Recheck；CheckTx_Recheck mempool normal recheck ≠ 外部新交易 / 去重保证不重放；Request type 栏 ≠ tx 栏 / Commit 后再验不需要读 Type（不变量 484）
- [`worked-example-applysnapusage-vs-info.md`](worked-example-applysnapusage-vs-info.md) — verify each chunk ≠ Only AppHash 可信任就交差 / 332 incremental interchangeable；all chunks accepted 后 Info ≠ 装块时就 Info 对了 / Transition interchangeable；unable to retrieve next chunk ≠ refetch 就齐 / REJECT_SNAPSHOT 回包 interchangeable（不变量 485）
- [`worked-example-chktxguardusage-vs-optional.md`](worked-example-chktxguardusage-vs-optional.md) — Guardian of the mempool ≠ Technically optional / 四门已经结算；every node runs CheckTx before letting into local mempool ≠ 已经流言 / Check 通过就是已进提案 / forever valid；before letting into its local mempool ≠ 已经保证不重放 / tx source bundled / Code≠0 rejected bundled（不变量 490）
- [`worked-example-chktxcodereject-vs-proposal.md`](worked-example-chktxcodereject-vs-proposal.md) — Code≠0 will be rejected / will not be broadcast ≠ 已经流言 / CheckTx 守卫 bundled；will not be included in a proposal block ≠ Check 通过就是已进提案 / forever valid / Finalize Code≠0 仍在块里；CometBFT attributes no other value ≠ CheckTx Data 已被引擎用了 / optional bundled / validate-no-apply bundled（不变量 489）
- [`worked-example-chktxsource-vs-recheck.md`](worked-example-chktxsource-vs-recheck.md) — may come from external user ≠ CheckTx_Recheck；may come from another node ≠ 已经从池里删掉 / CheckTx 过了就永远有效；external user or another node ≠ 已经保证不重放（不变量 488）
- [`worked-example-queryusage-vs-querystate.md`](worked-example-queryusage-vs-querystate.md) — Query for data at current or past height ≠ QueryState；Optionally return Merkle proof ≠ 已经对上 AppHash；Merkle proof self-describing type ≠ ProofOp 按键查（不变量 487）
- [`worked-example-chktxvalidate-vs-apply.md`](worked-example-chktxvalidate-vs-apply.md) — validates against current state ≠ ExecuteTxState / 将要执行的那份状态；does not apply state changes ≠ 已经改了状态 / Finalize interchangeable；Technically optional / Code≠0 rejected ≠ 四门已经结算 / CheckTx 过了就永远有效（不变量 486）
...
- [`worked-example-initchainusage-decide-vs-emptyset.md`](worked-example-initchainusage-decide-vs-emptyset.md) — app decide accept or different one ≠ Response empty/not empty 规则；Both Validators are ValidatorUpdate ≠ 已经改了集合；updating from empty set ≠ 空名单就没有集合（不变量 496）
- [`worked-example-infousage-persist-vs-lanebundled.md`](worked-example-infousage-persist-vs-lanebundled.md) — last_block persisted during Commit ≠ Info 握手 bundled；does not have to define lane_priorities ≠ Info 车道 bundled；lane_priorities empty iff default_lane empty ≠ Info 车道 bundled（不变量 497）
- [`worked-example-verifywhen-discard-vs-bundled.md`](worked-example-verifywhen-discard-vs-bundled.md) — discards invalid Precommit ≠ Verify When bundled；0-length with valid signature ≠ 空扩展仍会调 Verify bundled；step 1 before call ≠ already verified / last_commit（不变量 514）
...
- [`worked-example-extwhen-precommit-vs-bundled.md`](worked-example-extwhen-precommit-vs-bundled.md) — constructs Precommit ≠ construct CanonicalVote bundled；using both ≠ 只有 CanonicalVoteExtension / 只有 CanonicalVote；step 6 before broadcasts ≠ ExtendVote When 正式流程 bundled（不变量 512）
...
- [`worked-example-extwhen-fill-vs-bundled.md`](worked-example-extwhen-fill-vs-bundled.md) — sets extension into CanonicalVoteExtension.extension ≠ return extension bundled；populates other fields ≠ ExtendVoteRequest 栏 bundled；signs populated CanonicalVoteExtension ≠ ExtendVote When 正式流程 bundled（不变量 510）
...
- [`worked-example-loadsnapusage-retrieve-vs-bundled.md`](worked-example-loadsnapusage-retrieve-vs-bundled.md) — Used during state sync to retrieve ≠ LoadSnapshotChunk bundled；retrieve from peers ≠ ListSnapshots discover；retrieve snapshot chunks ≠ ApplySnapshotChunk chunk 栏（不变量 501）
- [`worked-example-listsnapusage-discover-vs-bundled.md`](worked-example-listsnapusage-discover-vs-bundled.md) — discover on peers during state sync ≠ ListSnapshots 空请求 bundled；See Snapshot data type for details ≠ Snapshot 类型 bundled（不变量 500）
- [`worked-example-offersnapusage-bootstrap-vs-bundled.md`](worked-example-offersnapusage-bootstrap-vs-bundled.md) — bootstrapping state sync / may accept or reject ≠ OfferSnapshot 请求 bundled；Upon accepting retrieve and apply chunks ≠ Offer 收下就已经装完；reject in chunk response prepared for further Offer ≠ Offer 收下之后 bundled（不变量 499）
- [`worked-example-infousage-defaultlane-vs-priorityzero.md`](worked-example-infousage-defaultlane-vs-priorityzero.md) — default_lane must be in lane_priorities ≠ Info 车道 bundled；priority 0 reserved for empty lane_id ≠ Info 车道 bundled（不变量 498）
- [`worked-example-initchainusage-vs-bundled.md`](worked-example-initchainusage-vs-bundled.md) — Called once upon genesis ≠ 崩溃后再调 InitChain；Response Validators empty → Request Validators ≠ 空名单就没有集合；Response Validators not empty → Response regardless of Request ≠ 应用 can decide bundled（不变量 495）
- [`worked-example-infousage-vs-handshakebundled.md`](worked-example-infousage-vs-handshakebundled.md) — Return information about the application state ≠ QueryState；Used to sync during handshake on startup or on recovery ≠ Info 握手 bundled；app_version included in Header of every block ≠ last_block persisted during Commit（不变量 494）
- [`worked-example-flushusage-vs-echo.md`](worked-example-flushusage-vs-echo.md) — Signals messages queued should be flushed to server ≠ Echo 测 implementation；Called periodically to ensure async requests are actually sent ≠ Echo 用来测实现；Called immediately for sync request returns when Flush response comes back ≠ Echo 回包 Message（不变量 493）
- [`worked-example-echousage-vs-flush.md`](worked-example-echousage-vs-flush.md) — Echo a string to test implementation ≠ 已经 Flush；Request Message string to echo back ≠ Response Message interchangeable；Response Message the input string ≠ 已经测实现就已经刷完（不变量 492）
...
- [`worked-example-prepevents-vs-finalize.md`](worked-example-prepevents-vs-finalize.md) — Prepare MAY 产出块/tx 事件 ≠ 已经在 PrepareProposalResponse 里交回；MUST 留到块决定之后 ≠ 已经 Process 时就交出去；经 FinalizeBlockResponse 交回 ≠ 已经是 CheckTx / ExecTxResult events（不变量 451）
- [`worked-example-commitretaincaution-vs-kept.md`](worked-example-commitretaincaution-vs-kept.md) — Use retain_height with caution ≠ retain_height 默认 0 就等于已经在剪；all nodes remove historical blocks ≠ 已经能从创世再装 / 已经开了 state sync 就交差；Historical blocks required for auditing / replay / light client ≠ persist signal bundled（不变量 491）
- [`worked-example-commitpersist-vs-finalize.md`](worked-example-commitpersist-vs-finalize.md) — Signal persist application state ≠ 已经在 Finalize 改了就已经落盘；expected persist at end of this call ≠ 已经 Commit 不带参数就等于已经落盘；Historical blocks required for auditing / replay / light client ≠ 已经 retain_height 默认 0 就等于已经在剪（不变量 481）
- [`worked-example-finmorepre-vs-timeout.md`](worked-example-finmorepre-vs-timeout.md) — includes processing time ≠ 已经是 timeout_commit；more precommits despite 2/3+ ≠ 已经决定 / 已经最终；after committing before next height ≠ 已经是槽位 / 1s 常量（不变量 480）
- [`worked-example-finwhenparts-vs-partial.md`](worked-example-finwhenparts-vs-partial.md) — Proposal + all block parts ≠ 已经只有 hash；2f+1 precommit same id(v) ≠ 已经 +2/3 prevote ExtendVote；decides block v ≠ 已经到了这一高就会调 Finalize（不变量 479）
- [`worked-example-finpersist-vs-commit.md`](worked-example-finpersist-vs-commit.md) — persist decision ≠ 已经 executes block v / 已经交差；calls FinalizeBlock ≠ 已经 persist outputs；synchronous call ≠ 已经决定触发 interchangeable（不变量 478）
...
- [`worked-example-finfill-vs-norepeat.md`](worked-example-finfill-vs-norepeat.md) — will fill up all fields ≠ 已经 Prepare / Process 给过就不用再 Finalize；even if already passed ≠ 已经字段名对得上就代表已经跑过 Process；all fields / 又填一遍 ≠ 已经 decided 和 proposed 就可以混用（不变量 473）
- [`worked-example-finproc-vs-allvalidators.md`](worked-example-finproc-vs-allvalidators.md) — When calling FinalizeBlock guarantee ≠ 已经每个验证者都跑过 Process；at least one non-byzantine ≠ 已经提议者 Process 过就代表全网都 Process 过；has run ProcessProposal ≠ 已经套用 candidate 就不需要 guarantee（不变量 472）
- [`worked-example-fincparam-vs-heffective.md`](worked-example-fincparam-vs-heffective.md) — returned for block H apply to H+1 ≠ 已经在块 H 生效；Changes to gas, size / Deterministic = Yes ≠ 已经只填一项；may be empty / keep current values ≠ 已经清掉参数（不变量 471）
- [`worked-example-findet-vs-replication.md`](worked-example-findet-vs-replication.md) — executes txs deterministically ≠ 已经可以像 Prepare 那样；app_hash MUST be deterministic ≠ 已经 next_block_delay 非确定就代表整门非确定；implementation MUST be deterministic ≠ 已经是 Req 11–12 / finfields bundled（不变量 470）
- [`worked-example-fndelay-vs-deterministic.md`](worked-example-fndelay-vs-deterministic.md) — next_block_delay 非确定 ≠ 已经 Finalize 必须确定；wallclock / NTP ≠ 已经是 timeout_commit；Commit 后再开下一高 / set to 0 ≠ 已经是槽位 / 已经最终（不变量 469）
- [`worked-example-finlock-vs-commit.md`](worked-example-finlock-vs-commit.md) — locks mempool ≠ 已经是 Commit 锁；calls Commit ≠ 已经引擎 persist 这三份；optionally recheck / unlock / start h+1 ≠ 已经是 Recheck（不变量 468）
- [`worked-example-finreturn-vs-header.md`](worked-example-finreturn-vs-header.md) — Application returns AppHash + tx outputs ≠ 已经印进本头；CometBFT hashes into ResultHash ≠ 已经 Code / Data 印进本头；CometBFT persists 这三份 ≠ 已经 Commit 落盘应用状态（不变量 467）
- [`worked-example-finexec-vs-decided.md`](worked-example-finexec-vs-decided.md) — Application executes block v ≠ 已经把 v 落成这一高的决定；executes block v ≠ 已经每个验证者都跑过 Process / 已经是 ExecuteTxState；executes block v ≠ 已经套用 candidate 就不需要再在 Finalize 执行（不变量 466）
- [`worked-example-finequiv-vs-abci1.md`](worked-example-finequiv-vs-abci1.md) — equiv to ABCI 1.0 BeginBlock/DeliverTx/EndBlock ≠ 已经是四门已经结算；收成一门 ≠ 已经没有 Prepare/Process；equiv ≠ 已经是含刚决定那块的字段 / 已经 Process 就不需要 Finalize（不变量 465）
- [`worked-example-fintxcode-vs-invalid.md`](worked-example-fintxcode-vs-invalid.md) — Code == 0 only if fully valid ≠ 已经 CheckTx 过了；fully valid ≠ 已经 Code != 0 那种没进块；回了 tx_results ≠ 已经 Finalize 改了就已经交差（不变量 464）
- [`worked-example-finreward-vs-slashed.md`](worked-example-finreward-vs-slashed.md) — can use decided_last_commit + misbehavior 定奖惩 ≠ 已经罚没；decided_last_commit 从刚决定那块拿到 ≠ 已经 proposed_last_commit；misbehavior 过错列表 ≠ 已经 VoteInfo 按到场定奖惩（不变量 463）
- [`worked-example-finht-vs-header.md`](worked-example-finht-vs-header.md) — Finalize height/time match header ≠ 已经验过块头；Request height/time 栏 ≠ 已经 Usage 那种 match header；Finalize match ≠ 已经是 ProcessProposal match interchangeable（不变量 462）
- [`worked-example-finnewfields-vs-gates.md`](worked-example-finnewfields-vs-gates.md) — Contains the fields of the newly decided block ≠ 已经是四门已经结算；newly decided block 的字段 ≠ 已经是 ProcessProposal 含执行所需全部信息；全部字段填齐即使 Prepare/Process 已经传过 ≠ 已经 decided 和 proposed 就可以混用（不变量 461）
- [`worked-example-fincand-vs-reexecute.md`](worked-example-fincand-vs-reexecute.md) — 确定执行 txs ≠ 已经交差；套用 candidate state ≠ 已经是 ExecuteTxState；同一块先前 Prepare/Process 执行过 ≠ 已经不用再在 Finalize 执行（不变量 460）
- [`worked-example-finvaldelay-vs-h2vote.md`](worked-example-finvaldelay-vs-h2vote.md) — H+1 只更新 Next ≠ 已经在 H+1 计票；H+2 ValidatorsHash 才生效 ≠ 已经在 H+1 换人；H+3 last_commit 才带新集合 ≠ 已经 proposed/decided_last_commit 就是新集合（不变量 459）
- [`worked-example-finempty-vs-mustprovide.md`](worked-example-finempty-vs-mustprovide.md) — 必须回四列 ≠ 已经改了集合；validator_updates 空则保持当前集合 ≠ 已经没有集合；consensus_param_updates 空则保持当前参数 ≠ 已经清掉参数（不变量 458）
- [`worked-example-verifyaccept-vs-req6.md`](worked-example-verifyaccept-vs-req6.md) — SHOULD 总是设 ACCEPT ≠ 已经正确进程交出的扩展必须 Accept；除非真的知道 REJECT 的活性代价 ≠ 已经拒整张 Precommit 是免费过滤；SHOULD Accept 默认策略 ≠ 已经 Requirement 6 已经测过（不变量 457）
- [`worked-example-procaccept-vs-req3.md`](worked-example-procaccept-vs-req3.md) — SHOULD 总是设 ACCEPT ≠ 已经 honest proposal 必须 Accept；除非真的知道 REJECT 的活性代价 ≠ 已经 REJECT 是免费过滤；SHOULD Accept 默认策略 ≠ 已经 Requirement 3 已经测过（不变量 456）
- [`worked-example-procreject-vs-assume.md`](worked-example-procreject-vs-assume.md) — REJECT 时共识假设收到的提案不合法 ≠ 已经当成块非法；验证者 prevote nil ≠ 已经 VerifyVoteExtension REJECT 拒整张票；REJECT 共识假设 ≠ 已经不能整块执行候选（不变量 455）
- [`worked-example-procht-vs-header.md`](worked-example-procht-vs-header.md) — Process height/time match header ≠ 已经验过块头；Request height/time 栏 ≠ 已经 Usage 那种对上了；Process match header ≠ 已经是 FinalizeBlockRequest 刚决定那块的字段（不变量 454）
- [`worked-example-procfull-vs-execute.md`](worked-example-procfull-vs-execute.md) — Contains all information needed to fully execute ≠ 已经执行那些交易；Request 八栏齐 ≠ 已经只有 PrepareProposalResponse.txs；含执行所需全部信息 ≠ 已经是 FinalizeBlockRequest 刚决定那块的字段（不变量 453）
- [`worked-example-proccand-vs-execute.md`](worked-example-proccand-vs-execute.md) — Process MAY 像 Finalize 整块执行 ≠ 已经交差 / 已经是 ExecuteTxState；须留 candidate state ≠ 已经改了已提交状态；read-only 处理 ≠ 已经改了上一份已提交状态（不变量 452）
- [`worked-example-misbtvp-vs-attendance.md`](worked-example-misbtvp-vs-attendance.md) — Misbehavior.total_voting_power 是 height 那一高验证者集合的总投票权 ≠ 已经 VoteInfo.validator.power / Misbehavior.validator.power 那种单个验证者权；Misbehavior.total_voting_power ≠ 已经按到场定奖惩；Misbehavior.total_voting_power ≠ 已经定了奖惩（不变量 450）
- [`worked-example-misbheighttime-vs-votetime.md`](worked-example-misbheighttime-vs-votetime.md) — Misbehavior.height 是过错发生的高度 ≠ 已经 Prepare/Process 请求 height；Misbehavior.time 是那一高已提交块的时间戳 ≠ 已经验过票上 Timestamp；height 和 time 一起 ≠ 已经定了奖惩（不变量 449）
- [`worked-example-misbvalidator-vs-voteinfo.md`](worked-example-misbvalidator-vs-voteinfo.md) — Misbehavior.validator 是过错验证者 ≠ 已经 slashed；Misbehavior.validator 只是 address+power 的 Validator 结构 ≠ 已经 VoteInfo.validator 那种按到场定奖惩；Misbehavior.validator ≠ 已经 ValidatorUpdate 那种已经改了集合（不变量 448）
- [`worked-example-misbtype-vs-slashed.md`](worked-example-misbtype-vs-slashed.md) — MisbehaviorType UNKNOWN 只是未知过错枚举 ≠ 已经归类成 DUPLICATE_VOTE；MisbehaviorType DUPLICATE_VOTE 只是双签枚举 ≠ 已经罚没；MisbehaviorType LIGHT_CLIENT_ATTACK 只是轻客户端攻击枚举 ≠ 已经 DUPLICATE_VOTE（不变量 447）
- [`worked-example-exectxevents-vs-header.md`](worked-example-exectxevents-vs-header.md) — ExecTxResult.events 是给交易建索引的类型键值事件 ≠ 已经印进本头；ExecTxResult.events 标成非确定 ≠ 已经像 Code/Data 那样必须确定；ExecTxResult.events 在 tx_results 里逐笔出现 ≠ 已经是 FinalizeBlockResponse.events / CheckTxResponse.events（不变量 446）
- [`worked-example-cifields-vs-notes.md`](worked-example-cifields-vs-notes.md) — CommitInfo.round 是提交轮 ≠ 已经按投票权排过；CommitInfo.votes 是上一验证者集合里各人的投票信息 ≠ 已经进了块；Fields 栏描述 round 和 votes ≠ 已经是 CommitInfo Notes 那套票序话（不变量 445）
- [`worked-example-cinotes-vs-order.md`](worked-example-cinotes-vs-order.md) — CommitInfo.votes 里的 VoteInfo 按投票权降序排 ≠ 已经进了块 / 已经交差；CometBFT 通过更新验证者集合的逻辑保证这个顺序、集合写入 store 时顺序也落盘 ≠ 已经由应用排过 / 已经是收到票时的顺序；造 CommitInfo 时从 store 再装集合 ≠ 已经从拟议块或已决块抽出 / 已经是 ExtendedCommitInfo Notes 那套话就已经是同一句（不变量 444）
- [`worked-example-validatorusage-vs-gates.md`](worked-example-validatorusage-vs-gates.md) — Validator 装在 CommitInfo 里用于 ProcessProposal ≠ 已经 PrepareProposal 里的 ExtendedCommitInfo / 已经是 local_last_commit 同一份；Validator 装在 CommitInfo 里用于 FinalizeBlock ≠ 已经 Process 里的 proposed_last_commit 同一路 / 已经是 decided_last_commit 就已经交差；Validator 装在 ExtendedCommitInfo 里用于 PrepareProposal ≠ 已经 Process/Finalize 里的 CommitInfo 同一路 / 已经可以 interchangeable（不变量 443）
- [`worked-example-viusageavail-vs-extractpath.md`](worked-example-viusageavail-vs-extractpath.md) — VoteInfo 与 ExtendedVoteInfo Usage 都写 allowing for rewards based on validator availability ≠ 已经奖罚完 / 已经交差；VoteInfo 通常从拟议块或已决块抽出 ≠ 已经从本进程抽出 / 已经是 Prepare 里的 ExtendedVoteInfo；ExtendedVoteInfo 从本进程 CometBFT 数据结构抽出 ≠ 已经 typically extracted from proposed or decided block / 已经是 Process/Finalize 里的 CommitInfo 同一路（不变量 442）
- [`worked-example-extcinotes-vs-order.md`](worked-example-extcinotes-vs-order.md) — ExtendedCommitInfo.votes 里的 ExtendedVoteInfo 按投票权降序排 ≠ 已经进了块 / 已经交差；CometBFT 通过更新验证者集合的逻辑保证这个顺序、集合写入 store 时顺序也落盘 ≠ 已经由应用排过 / 已经是收到票时的顺序；造 ExtendedCommitInfo 时从 store 再装集合 ≠ 已经从拟议块或已决块抽出 / 已经是 CommitInfo Notes 那套话就已经是同一句（不变量 441）
