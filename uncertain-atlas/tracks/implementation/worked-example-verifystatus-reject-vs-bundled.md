...
1. **看见 VerifyStatus 的 `REJECT` 表示应用认为扩展非法、共识会拒掉整张票 / 看见回了 `REJECT` 不是已经会发 Prevote nil，也不是已经 VerifyStatus bundled（434） interchangeable / 已经验过扩展 interchangeable / 已经 MUST Accept interchangeable，也不是已经 ProcessProposalResponse.status is REJECT bundled（430 / 533 余量） interchangeable / 已经 prevote nil interchangeable / 已经 Process REJECT = prevote nil 不是免费过滤 bundled（33） interchangeable，也不是已经 ProposalStatus REJECT 会发 Prevote nil bundled（376） interchangeable / 已经当成块非法 interchangeable，也不是已经 Verify When step 3 REJECT discard bundled（517） interchangeable / 已经丢掉 Precommit interchangeable，也不是已经 VerifyStatus ACCEPT bundled（542 余量） interchangeable / 已经收下这张票 interchangeable。**  
...
怎样做写 `VerifyStatus`、怎样拒整张 Precommit 是规范里的做法，本页不抄。VerifyStatus bundled（434）、VerifyStatus UNKNOWN/ACCEPT（537/542 余量）、ProposalStatus REJECT（376）、Process REJECT prevote nil（430/533）是另外那套，本页不抄。
...
- VerifyStatus 的 UNKNOWN 一律是错。那是不变量 541（434 item 1 余量）。
- VerifyStatus 的 ACCEPT 会收下这张票。那是不变量 542（434 item 2 余量）。
- VerifyVoteExtensionResponse.status REJECT rejects whole vote not block invalid。那是不变量 539（433 item 1 余量）。
