...
- newly decided block 不是 proposed block / ProcessProposal 含执行所需全部信息。那是不变量 565（474 item 2 余量）。
- fields of the newly decided block 不是 height/time match header 就代表对象已经分清。那是不变量 566（474 item 3 余量）。
- FinalizeBlock Contains newly decided block fields not already settled（461 余量）。那是不变量 555。
