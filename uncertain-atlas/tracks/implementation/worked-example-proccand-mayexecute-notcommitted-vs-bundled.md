...
怎样做实现 candidate 缓存、怎样在 Finalize 套用 是规范里的做法，本页不抄。ProcessProposal 候选执行 bundled（452）、candidate state must be kept（544 余量）、read-only checks/processes（545 余量）是另外那套，本页不抄。
...
- Process REJECT assume not can't execute candidate。那是不变量 542。
