...
**边界：** 不写 IBC 反向高度的未完成问题；不抄某一链的解绑秒数；检测/问责算法后置。Ethereum 弱主观性是亲戚：[`../finality/worked-example-weak-subjectivity.md`](../finality/worked-example-weak-subjectivity.md)。朝前 lunatic 与「验过头 ≠ 能交证据」见 [Alderfly](../failure-museum/alderfly.md)。ICS-23 验绿 ≠ 叶子已在原树见 [Dragonberry](../failure-museum/dragonberry.md)。
