...
- [queryheight-notfresh-sold-as-bundled](queryheight-notfresh-sold-as-bundled.md) 是 height 默认 0 回最新已提交不是已经新鲜 not already fresh / not already caught-up / not already info-handshake not already fresh / not already caught-up / not already info-handshake 正式三事（371 item 2），不是本页 bundled 全段 alone。
- [queryheight-notapphash-sold-as-bundled](queryheight-notapphash-sold-as-bundled.md) 是 这个 height 是含 Merkle 根的那块、代表 Height-1 提交后的状态不是已经印进本头 AppHash not already apphash / not already proof / not already settled not already apphash / not already proof / not already settled 正式三事（371 item 3），不是本页 bundled 全段 alone。
