...
- [extend-notnil-sold-as-bundled](extend-notnil-sold-as-bundled.md) 是 ExtendVote 只在即将广播非 nil Precommit 时才叫 not already signed-nil / not already prevote-calls / not already vote-has-ext 正式三事（350 item 2），不是本页 bundled 全段 alone。
- [extend-notperheight-sold-as-bundled](extend-notperheight-sold-as-bundled.md) 是一轮只能交出一份扩展 not already per-height / not already re-extend-round / not already req6-accept 正式三事（350 item 3），不是本页 bundled 全段 alone。
