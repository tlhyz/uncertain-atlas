...
- [querypath-notrequired-sold-as-bundled](querypath-notrequired-sold-as-bundled.md) 是规范建议允许 /accounts / /votes 这类查询不是已经是正常运转必须有 not already required / not already replicated / not already fresh 正式三事（377 item 3），不是本页 bundled 全段 alone。
- [querypath-notengine-sold-as-bundled](querypath-notengine-sold-as-bundled.md) 是 path 按 URI 路径解释、/store 必须按键查不是已经是引擎在用 not already engine / not already filter / not already settled 正式三事（377 item 2），不是本页 bundled 全段 alone。
