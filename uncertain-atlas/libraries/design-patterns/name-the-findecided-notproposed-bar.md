...
3. **decided_last_commit from decided block not can use means slashed 不是 567 can use not slashed：** 看见 decided_last_commit 从刚决定那块拿到不是已经可以用这两列定奖惩 / 已经 slashed，不是 463 bundled interchangeable / 567 can use not slashed interchangeable / 363 回包义务 interchangeable。
