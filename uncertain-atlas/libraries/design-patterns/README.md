- [`name-the-queryprove-notkey-bar.md`](name-the-queryprove-notkey-bar.md) — Query 回包 proof_ops 是按请求回的、要对这一高 AppHash 验的序列化证明不是已经是按键查 not already key / not already matched / not already settled 正式三事（383 余量）（不变量 897）
