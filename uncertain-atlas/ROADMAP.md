...
2. CometBFT ABCI++ 不变量拆完（目标 ~793 句；当前 ~853）。
3. 决策矩阵「不确定候选」列填完**建议档**（非最终选型）。
4. 对抗语料 C01–C831+ 有 runner，能批量扫描文案/测试钩子。
...
| P1-3 | ABCI++ 拆完 677–853 | AUDIT_LOG 连续；invariants README 更新 |
...
下一批默认：**P1-3 ABCI++ 不变量 854+**（继续 cometbft 拆句）。  
