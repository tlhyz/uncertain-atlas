...
2. CometBFT ABCI++ 不变量拆完（目标 ~793 句；当前 ~1379）。
3. 决策矩阵「不确定候选」列填完**建议档**（非最终选型）。
4. 对抗语料 C01–C1357+ 有 runner，能批量扫描文案/测试钩子。
...
下一批默认：**P1-3 官方三事 1380+**（回收簇已拆完；继续以太坊仍捆着的官方对象：EIP-140 REVERT，或 P3-3 / P1-5）。  
