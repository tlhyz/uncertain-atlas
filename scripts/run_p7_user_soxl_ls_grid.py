#!/usr/bin/env python3
"""P7-05: user 5x moving grid, 5k long + 5k short SOXL — ±20U and ±20%."""
...
from src.analysis.user_moving_grid import run_user_ls_pair

OUT = ROOT / "outputs" / "experiments" / "soxl_user_ls_grid"
START = "2026-07-16"
END = "2026-09-11"
...
def main() -> int:
    ...
    summary = {"generated": date.today().isoformat(), "window": f"{START}->{END}", "n_bars": int(len(bars)), "variants": {}}
    for mode in ("usdt", "pct"):
        print(f"[p7-05] tick base range_mode={mode}", flush=True)
        rep = run_user_ls_pair(bars, range_mode=mode, fill_engine="tick", fee_preset="base", get_trades=gt)
        daily = rep["daily"]
        daily.to_csv(OUT / f"{mode}20_daily.csv", index=False)
        summary["variants"][mode] = _strip(rep)
        print(json.dumps(summary["variants"][mode], indent=2, default=str), flush=True)

    path = OUT / "summary.json"
    path.write_text(json.dumps(summary, indent=2, default=str), encoding="utf-8")
    print(f"[p7-05] wrote {path}", flush=True)
    return 0
