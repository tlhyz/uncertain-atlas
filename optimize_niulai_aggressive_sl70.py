#!/usr/bin/env python3
"""Aggressive dual-hedge futures Martingale optimize for 牛来_USDT WITH stop_loss_pct=0.70."""
...
OUT_JSON = ROOT / "opt_niulai_aggressive_sl70_results.json"
OUT_MD = ROOT / "opt_niulai_aggressive_sl70_report.md"
OUT_LOG = ROOT / "opt_niulai_aggressive_sl70_run.log"
...
def write_report(payload: dict) -> None:
    ...
    shot = payload["screenshot_proxy_sl70"]
    ...
    lines: list[str] = []
    lines.append("# 牛来_USDT 激进双开马丁优化报告（止损 SL70 / 用户风格）")
    lines.append("")
    ...
    lines.append("- **止损 SL70（本轮新增）**：见 §0.1")
    ...
    lines.append("")
    lines.append("- 参数：`stop_loss_pct = 0.70`")
    lines.append("- 触发条件：开仓周期内 **未实现盈亏 uPnL / 该机器人投资额 initial_margin ≤ −0.70**")
    lines.append("- 动作：按触发价强制平掉**整仓**，兑现该周期亏损，计入 `stop_outs`，然后**允许开启新一轮**（继续跑，不是永久停机）")
    lines.append("- 意图对齐：截断单轮 −70% 投资额级别的浮亏，避免拖到强平；周期结束后重启")
    lines.append("")
    ...
    lines.append(f"- 若要把「亏约 70% 投资额」粗映射成 Gate 价格止损%：在仓位名义≈投资额×杠杆（满用）时，约 **{price_eq*100:.1f}% 价格偏离均价**（= 70%/5x）")
    lines.append("- **本报告 Gate 填写表仍写 止损=70%**（按你要求的字段值）；请知悉：若 Gate UI 把 70% 解释成价格%，经济含义远大于「亏 70% 保证金」，几乎等同极宽止损；若要接近本回测经济效果，Gate 价格止损可考虑填约 **14%**，或确认 UI 是否支持按投资额/保证金亏损% 止损")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 2. 最优双开 — Gate 机器人填写表（含止损=70%）")
    lines.append("")
    lines.append(
    ...
    lines.append(f"| 止损 | **70%** | 回测按 uPnL/投资额≤−70% 触发；Gate UI 若为价格%请见 §0.1 |")
    ...
    lines.append("- 两侧均 **5x** + **max_adds 深缓冲** + **止损 70%**")
    lines.append("")
    lines.append("## 3. 双开回测现金流（事实，含 SL70）")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 4. 对比：上一轮激进双开（无止损） vs 本轮 SL70")
    lines.append("")
    if prev:
        pl, ps = prev["long"], prev["short"]
        lines.append("| | 无SL LONG | 无SL SHORT | 无SL合计 | SL70 LONG | SL70 SHORT | SL70合计 |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|")
        lines.append(
            f"| cashflow | {pl['realized_cashflow']} | {ps['realized_cashflow']} | "
            f"{prev['combined_realized_cashflow']} | {lo['realized_cashflow']} | {sh['realized_cashflow']} | "
            f"**{dual['combined_realized_cashflow']}** |"
        )
        lines.append(
            f"| cycles | {pl['cycles']} | {ps['cycles']} | {prev['combined_cycles']} | "
            f"{lo['cycles']} | {sh['cycles']} | **{dual['combined_cycles']}** |"
        )
        lines.append(
            f"| stop_outs | 0 | 0 | 0 | {lo['stop_outs']} | {sh['stop_outs']} | "
            f"**{lo['stop_outs']+sh['stop_outs']}** |"
        )
        lines.append(
            f"| net_pnl | {pl['net_pnl']} | {ps['net_pnl']} | {prev['combined_net_pnl']} | "
            f"{lo['net_pnl']} | {sh['net_pnl']} | **{dual['combined_net_pnl']}** |"
        )
        lines.append(
            f"| maxDD | {pl['max_drawdown']} | {ps['max_drawdown']} | {prev['combined_max_dd_proxy']} | "
            f"{lo['max_drawdown']} | {sh['max_drawdown']} | {dual['combined_max_dd_proxy']} |"
        )
        lines.append(
            f"| 强平 | {pl['liquidated']} | {ps['liquidated']} | — | {lo['liquidated']} | {sh['liquidated']} | — |"
        )
        lines.append("")
        cf_delta = dual["combined_realized_cashflow"] - prev["combined_realized_cashflow"]
        lines.append(
            f"- **现金流差额（SL70 − 无SL）**：{round(cf_delta, 4)} USDT"
            f"（止损会截断深亏轮，也会截断本可靠加仓摊薄后反弹收回的轮次）"
        )
        lines.append(
            f"- **参数是否同一套**：{'否，本轮在 SL70 约束下重新搜索' if prev.get('params_differ', True) else '是'}"
        )
    else:
        lines.append("（未找到上一轮 `opt_niulai_aggressive_results.json`，跳过对比）")
    lines.append("")
    lines.append("### 4.1 截图代理（同开 SL70） vs 本轮优化")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 5. 单侧 Top 速览（SL70 风格内）")
    lines.append("")
    ...
    lines.append("- JSON：`opt_niulai_aggressive_sl70_results.json`")
    lines.append("")
...
def main():
    ...
    print("Aggressive LONG search @950 with SL70 ...")
    ...
    print("Aggressive SHORT search @1380 with SL70 ...")
    ...
    present["mode"] = best_dual["mode"] + "_aggressive_dual_sl70_buf90"
    ...
    present = {
        ...
        "mode": "asymmetric_aggressive_dual_sl70_buf90",
        "long": mir_long,
        "short": mir_short,
    ...
    # Also re-run previous best params WITH SL70 for apples-to-apples param holdout
    prev_with_sl = None
    if prev:
        plp, psp = prev["long"]["params"], prev["short"]["params"]
        prev_long_sl = run_one(
            df,
            "long",
            plp["initial_margin"],
            plp["leverage"],
            ...
            plp["take_profit_pct"],
            plp["max_adds"],
            plp["base_order_quote"],
            plp.get("sizing", "prev") + "|sl70_holdout",
            stop_loss_pct=STOP_LOSS_PCT,
        )
        prev_short_sl = run_one(
            df,
            "short",
            psp["initial_margin"],
            psp["leverage"],
            ...
            psp["take_profit_pct"],
            psp["max_adds"],
            psp["base_order_quote"],
            psp.get("sizing", "prev") + "|sl70_holdout",
            stop_loss_pct=STOP_LOSS_PCT,
        )
        prev_with_sl = {
            "combined_realized_cashflow": round(
                prev_long_sl["realized_cashflow"] + prev_short_sl["realized_cashflow"], 4
            ),
            "combined_cycles": prev_long_sl["cycles"] + prev_short_sl["cycles"],
            "combined_stop_outs": prev_long_sl["stop_outs"] + prev_short_sl["stop_outs"],
            "combined_net_pnl": round(prev_long_sl["net_pnl"] + prev_short_sl["net_pnl"], 4),
            "long": prev_long_sl,
            "short": prev_short_sl,
            "note": "previous aggressive params re-run with SL70 (not re-optimized)",
        }
    ...
    payload = {
        "meta": {
            ...
            "style": "aggressive_dual_hedge_small_size_sl70",
            "stop_loss": {
                "pct": STOP_LOSS_PCT,
        ...
        "screenshot_proxy_sl70": shot_sl,
        "screenshot_proxy_sl70_alt": shot_sl_alt,
        "screenshot_proxy_no_sl": shot_nosl,
        "compare_prev_no_sl": prev,
        "compare_prev_params_with_sl70": prev_with_sl,
        "raw_best_maybe_liq": {
